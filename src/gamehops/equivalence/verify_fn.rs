...
use crate::writers::smt::patterns::oracle;
use crate::{
    ...
    EquivalenceSmtDriver<'a, Backend, Proj>
{
    ...
    fn do_verify_oracle<UI: TheoremUI + Send>(
        ...
        let verify_oracle_claims = claims
            ...
            .map(|claim| -> Result<()> {
                self.verify_oracle_claim(ui.clone(), smt, oracle.name(), claim, claim_group)
            });
    ...
    fn verify_oracle_claim<UI: TheoremUI>(
        ...
        smt.extend(
            self.eqctx
                .emit_randomness_mapping_declarations(oracle_name),
        );
        smt.push(self.eqctx.emit_randomness_mapping_condition(oracle_name));
